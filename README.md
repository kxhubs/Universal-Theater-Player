...
- 新增 Javday、Javrate 及其嵌套播放器的影院模式适配。
- 修复部分网站误进入浏览器原生全屏、视频比例异常及进入时先缩小再放大的问题；现在会从首帧开始等比例铺满网页视口。
- 进入和退出影院模式时保留原播放器及 iframe，不再刷新页面或重置播放进度。
- 隐藏网站原生进度条和中央播放按钮，恢复单击显示悬浮时间轴与自定义暂停提示的交互。
- 加强跨 iframe 消息校验、播放器状态恢复和事件清理。

### 5.1.10.10

- 手机竖屏影院模式下，底部改为常驻迷你进度条。
- 点击视频或迷你进度条时显示完整控制栏，短暂停留后自动收起，减少遮挡画面。
